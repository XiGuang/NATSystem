...
template <class ElemType>
ElemType& LinkList<ElemType>::InsertElem(const ElemType &e)
// 操作结果：在单链表的表尾位置插入元素e
{
	Node<ElemType> *p, *q;
...
template<class ElemType>
ElemType &LinkList<ElemType>::operator[](int position) const {
    if (position < 0 || position >= length)
        throw out_of_range("下标超范围！");
    Node<ElemType> *p = head->next;
    int count;
